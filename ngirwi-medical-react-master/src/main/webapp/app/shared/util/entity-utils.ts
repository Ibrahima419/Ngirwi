...
// FCFA formatting with 0-decimal rounding
export const formatFcfa = (value?: number | null): string => {
  if (value === undefined || value === null || isNaN(value as any)) return '0 FCFA';
  try {
    const rounded = Math.round(value);
    return new Intl.NumberFormat(undefined, { maximumFractionDigits: 0 }).format(rounded) + ' FCFA';
  } catch {
